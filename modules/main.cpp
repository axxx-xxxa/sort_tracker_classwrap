...
void TestSORT(string seqName, bool display)
{
	...
	while ( getline(detectionFile, detLine) )
	{
		...
		std::cout<<"tb is : "<<tb.frame<<" "<<tb.id<<" "<<tb.box<<std::endl;
...
int main()
{
	...
	VecTest("PETS09-S2L1", detData);
	TestSORT(detData, false);
...
void virtual_data(vector<vector<TrackingBox>>& detFrameData, string seqName) {
	cout << "Processing " << seqName << "..." << endl;
	...
	while (getline(detectionFile, detLine))
	{
		...
		//sorter.update_Vectest(tb, resultsFile);
	}
...
{
	ofstream resultsFile;
	string resFileName = "output/res.txt";
	resultsFile.open(resFileName);
	...
	int initres = sorter.init();
	virtual_data(detFrameData, "PETS09-S2L1");
	
	int updateres = 0;
	for (auto data : detFrameData) {
		updateres = sorter.update_Vectest(data, resultsFile);
	}

	std::cout << "class sort" << std::endl;
